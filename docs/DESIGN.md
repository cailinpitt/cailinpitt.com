...
Why each part of the site works the way it does, feature by feature, including the things not to
change and why. [`../README.md`](../README.md) is the overview and how to run it;
[`../RUNBOOK.md`](../RUNBOOK.md) is the commands. Read the section here that covers whatever you're
about to touch.
...
Every post is available as its source file, as are the two pages that are themselves a Markdown
file — `/colophon.md` and `/projects.md`.
...
- **A `.md` sitting directly in `content/` is a page whose route is its name**, and its source
  ...
  The colophon's source keeps its `{{placeholders}}` unfilled.
- On the page, the **Markdown** toggle above the body (`src/components/PostSource.tsx`) swaps the
  ...
  from the loader data the page already renders from, so it can't disagree with what's on screen.
  The inline view has no frontmatter; the file does.
...
- Body images read `/images/...` in the source, not the R2 URLs they're rewritten to at render
  time, so the source matches what you'd paste into a post.
- The dev server serves the same URLs (middleware in `vite.config.ts`), so the link works in dev too.

### Provenance

Each post ends with a line about its own file — "Edited 6 times since August 1, 2026" — that
expands into the list of commits, each linking to GitHub. `/colophon` carries the same line, being
a Markdown file like any post.

The counting rule matters here. History starts in June 2026: 31 posts arrived from Squarespace in
one commit and were reformatted in a second the next day. Counting those would report every
pre-2026 post as revised twice, for repo plumbing rather than writing. So **a commit touching
`BULK_POSTS` (3) or more posts is shown but not counted**, and a post never edited on its own just
says when it turned up: "Imported June 20, 2026", with a note explaining the migration. Bulk rows
in the list are marked `31 POSTS`.

Each commit's subject opens the **diff** of what it changed in this post, as track changes: what
came out struck through, what went in marked, in `<ins>`/`<del>` so the change is in the markup and
not only the color.
...
| **Similarity floor** | Below 30% shared text the pair is a rewrite, not an edit, and shows as whole `−`/`+` lines rather than marking every token |

The commit that added a post has no diff. Diffs are collected with one `git show` per commit, not
per commit per post — the reformat that touched 31 posts is one patch.
...
> **Keep `images/` at the repo root, not under `public/`.** Vite copies all of `public/` into
> `dist/`, but nothing on the built site ever requests a `/images/...` path — `imageUrl()` in
> `src/lib/images.ts` rewrites every one to R2 at render time. Keeping renditions in `public/` would
> upload ~240 MB to GitHub Pages on every deploy for files no page asks for. It's a staging area:
> `images:sync` writes into it, `images:upload` pushes it to R2. Paths come from `scripts/paths.mjs`.
>
...
> GitHub Pages for a file that isn't deployed.
...
- **Compresses.** Each original becomes four WebPs in `images/<folder>/`: 2560px full size, plus a
  ...
  file already ending in one of those suffixes is a prior output and is skipped when the manifest is
  rebuilt.
...
- **Records EXIF** from the original: date, camera, aperture, shutter, ISO, focal length, coarse
  location. Read once, then left alone.
...
`/now` is a [now page](https://nownownow.org/about/): what has my attention right now, as opposed
to what `/blog` or `/projects` shows. Prose lives in **`content/now.md`** — edit that, not
`src/pages/Now.tsx`.

- **The "Updated" date comes from `git log`, not frontmatter** — a hand-maintained date is easy to
  forget to bump. It's the newest commit to touch the file, read from the same
...
> **Three pages share this shape** — `/now`, `/uses`, and `/colophon` each render one Markdown file
> with a source toggle and a history panel. The JSX is copied three times; extract it if a fourth
> turns up.
...
endpoint — everything is derived in `src/lib/wrapped.ts`, where the thresholds live. Cards and
traits drop when inputs are missing, genre and geography claims need >=50% coverage, and a period
under 50 scrobbles gets no story at all.
...
filled from `artists.first_uts` by `markNewEntries()`. Conflating them contradicts the Discovery
section, which means first-ever play.

**Aggregation happens on the cron, never on a request** — this is what makes it affordable. A
...
visitor can do causes a D1 scan, so cost is flat regardless of traffic.
...
- Period pages reuse `/og/listening.jpg` rather than generating their own card — ~360 of them,
  ...
  shares, the same trade the photo permalinks make. **Wrapped pages keep bespoke cards** — there are
  only six.
...
The tight window is right for an hours-listened figure but wrong for "what was playing on this
ride": a ride with real stops in it — an errand run, a day working from three coffee shops — is one
outing, and the music kept playing through the stops. **21 of the last 100 activities ran 3x longer
than their moving time**, so that's the normal case. One 5h41m ride was showing 5 of its 37 tracks.

The failure modes aren't symmetric: over-including costs a slightly longer list, under-including
drops most of the answer. So the expander takes the recorded span, falling back to `windowSeconds`
only when an older Worker sends no `elapsedTime`. Both the batched count and the expanded list go
through `soundtrackWindow()`, so label and list can't disagree.
...
- **History before those 50** comes from a one-off CSV import: `npm run watching:backfill
  ...
  rows.
- **Reviews are not stored**, and cards link to `letterboxd.com/film/<slug>`, never to the diary
  entry under `/<member>/`.
...
- **Nothing user-facing names the source.** The page, nav, terminal view, and row copy don't say
  where the data comes from; the Worker and these docs say Strava freely. Keep that in mind when
  editing copy.
...
- **The export has no local timestamp**, only UTC, so backfilled dates are approximated with a fixed
  ...
  every stored row and takes Strava's own per-activity local date — required after a backfill.
...
- **Heart rate comes in on the activity summary**, so it costs no extra Strava requests. A row reads
  "♥ 145 avg · 178 max" only when a monitor was worn; both figures are labelled since a bare bpm
  number is ambiguous. The columns are nullable; zero means no reading. Adding it to an existing
  database needs `worker-moving/schema-v2.sql` then one `npm run moving:sync -- --refresh` to fill
  in history — the incremental sync only rewrites the last week.
...
Owned by `worker-comments/` — same write path and instant-publish design as the guestbook, scoped
by `post_path` instead of sitewide.
...
pagination. There are no galleries — `/2019`, `/latest`, `/past-work` and the like now 404.
...
- **Each tile paints its photo's average color** while the image loads (`tint` in the manifest,
  computed by `scripts/tint.mjs` during `images:sync`), so the feed shows photographs arriving
  rather than identical gray squares. 7 bytes per photo.
- **A floating marker names the year** at the top of the feed as you scroll — measured off the
  year-break tiles that already carry the `#y2019` anchors, needs no extra markup, and degrades to
  nothing without JavaScript.
- **The whole feed is in the prerendered HTML.** ~500 lazy `<img>` tags gzip to ~20 KB, giving a
  page that works without JavaScript, is Cmd-F searchable, and has real year anchors
...
- Positions are rounded to ~0.7 miles, which the page states.
...
- **Depth is set by the listening days**, the densest stream. Everything else is filtered into that
  window; "load older" pulls another block. Once listening is exhausted the rest of the history
  shows — currently ~77 rows back to 2015.
...
- **Concerts are not fetched** — they come from build-time static data (`src/lib/concerts.json`)
  alongside posts and photos. The full archive loads at once, so there's no cursor; the same `floor`
  filter as posts/photos applies but never has more to top up.
...
- **One stream failing costs that stream and nothing else.** This is the only page that reads every
  Worker. It loads them with `Promise.allSettled`: a missing stream contributes no rows, the day is
  assembled from the rest, and the error state appears only when *nothing* answered. `topUp()`
  swallows for the same reason and hands the cursor back unchanged so the next click retries. (With
  `Promise.all`, one dead endpoint rejected the batch and took the whole page down — as an
  undeployed notes Worker once did.)
- **Notes carry their own text** rather than a link — a note is at most 480 characters. A note's
...
- **"On this day"** surfaces days already loaded that share today's month and day across past years,
  ...
  history loaded often turns up nothing. The alternative would be a "how far back does this
  note/photo/activity go" endpoint on every Worker; `/listening`'s own On this day
  (`worker-listening`'s `/on-this-day.json`) can afford a real query because scrobbles have the
  depth for it.
- **The homepage previews the last five days of it** — see [Homepage](#homepage). Same
  `buildTimeline()`, so a day reads the same on both.
- **`/timeline/:date` is linked from the thing that happened**: every blog post, photo page, and
  single note links to its own day ("That day →").
...
**The split from `/blog` drives the design.** A post is an essay — title, tags, reading time,
related posts, a provenance line, a Markdown source. A note is a sentence, so the page has none of
that: one narrow column, a hairline between notes, and a timestamp that is also the permalink. If a
note needs to be a real page, it's a post, and belongs in `content/blog/`.

- **Notes live in D1, not in git** — the one kind of content on the site with no file behind it.
  See [`worker-notes/README.md`](../worker-notes/README.md#the-main-design-decision): a
  note that had to wait for a green CI run before appearing wouldn't get written, so notes are
  fetched at runtime like `/listening` and `/reading` rather than prerendered.
- **A note's shareable permalink is `cailinpitt.com/notes/<id>`**, rendered by the Worker on
  ...
  (`notePath()`, pinned by a test) is used for internal navigation — an anchor rather than a route,
  so following one is a client-side jump rather than a round trip through the Worker. Full detail:
...
- **A note can optionally reference one other thing on the site** — a photo, a moving activity, or a
  blog post, picked from `/notes/compose` — shown as a "re: …" line under the note's text, linked
  where the referenced thing has a permalink. Resolved from whatever the current page already has
  loaded, never a fetch made just to label a reference — see `src/lib/notesContext.ts`.
- **A live filter narrows the feed** to notes already loaded on the page — same substring match as
  the `/blog` filter below, and for the same reason: there's little enough to search that filtering
  what's on screen is enough.
...
- **Notes are plain text, not Markdown.** The only formatting needed is a working link: `segments()`
  parses bare URLs into a data structure `NoteText.tsx` maps over — no HTML string in the pipeline,
  no `dangerouslySetInnerHTML`. The URL boundary is fiddly: `(https://example.com)` shouldn't link
  the bracket, `…/wiki/Tag_(2018_film)` should keep it. `trimTrailing()` counts brackets to tell
  them apart, and a test asserts the segments rejoin to exactly the input.
- **Editing and deleting are first-class**, since publishing from a phone means publishing typos. An
  edit stamps `edited_at` and the page shows "edited" so a permalink doesn't silently change.
- **Its RSS is served by the Worker** at `notes.cailinpitt.com/feed.xml`, separate from the site's
...
> **That token is in `localStorage` on a public page, a deliberate trade.** A CLI would be safer but
> useless here — the point of the feature is publishing without the repo to hand. The site ships no
> third-party JavaScript and no user-generated HTML, so the realistic threat is a borrowed laptop,
> not injection; "Forget the token on this device" and rotating the Worker secret cover that. The
> page is `noindex`, absent from `sitemap.xml`, and `unlisted` in ⌘K. **The Worker enforces the
> token**, never the UI.
...
**A route missing from `PAGES` fails `tests/command-palette.test.ts`**, so the compose route can't
just be left out. But the palette's resting state renders every listed page. So the entry carries
`unlisted: true`, and the resting list, search index, and haystacks are built from `LISTED_PAGES`
rather than `PAGES`. Two tests pin it: the entry is in `PAGES` and not in `LISTED_PAGES`.
...
- **"Right now"** — a strip of present-tense pieces the day-grained timeline below can't carry: the
  now-playing bar (polls `/now.json` every 60s), the current book (`/now.json` from reading), and
  the 90-day scrobble sparkline. Each renders nothing until its fetch lands and nothing if it fails;
  the whole strip is hidden until at least one has data.
- **Timeline preview** (`HomeTimeline`). The last five days of [/timeline](#timeline), each a small
  icon-led event list (`dayEvents()`, capped at five with a "+N more") linking to that day, under an
  "N years ago today" line. Every stream `/timeline` shows is represented, with the same per-stream
  colour accents. Built by `useHomeTimeline`
...
- **`lastBuildDate` is the newest post's date, not the build time** — every deploy rewrites the
  file, and a moving timestamp would signal new content on every deploy.
...
Worker-backed). "Logs" rather than "Doing" because the rest of the site uses that word (the homepage
links read "Listening log →"). Grouping also keeps the row from growing a link per page.

**Timeline** sits directly after Logs — it's the union of them — and is set apart from the rest of
the row. Blog posts, photo pages, and single notes each carry a link to their own day on it ("That
day →").
...
announcement, and keyboard activation. `src/components/NavMenu.tsx` adds the rest: close on
navigation, on Escape, and on an outside click. The summary carries `aria-current="page"` when any
of its own pages is the one being viewed.
...
browser chrome matches the page. A media query can't see a manual override, so `ThemeToggle`
rewrites both tags with the resolved `--bg` when a theme is forced. The value is read from the
stylesheet rather than duplicated in JS, so it happens after mount — in the pre-paint script the CSS
isn't loaded and `--bg` reads empty. Cost: an overridden theme keeps the media-matched tint for the
first few frames.

## Home screen / installing

`public/site.webmanifest` has **no `start_url`**, deliberately.
...
`"scope": "/"` is the other half and is not optional. `scope` defaults to the directory of the
...
One consequence: each page pinned this way is the same app identity to the browser (same manifest
URL), so pinning several pages gives several icons, not several independent apps.
...
- **`worker-photos/` is thin**: authenticate, check the file, store it, fire a dispatch. It accepts
  the photo as a multipart `photo` field or the raw body, and reads alt text and date from form
  fields, `X-Photo-*` headers, or the query string. It can't do more — renditions need `sharp`,
  which doesn't run in a Worker, and the manifest is a file in git.
...
- **Publishing takes a deploy.** In exchange a phone photo is like any other: prerendered,
  permalinked, with a social card.
...
| Test | Guards against |
|---|---|
| `frontmatter`, `posts`, `tags`, `colophon` | Silent failures in the pure functions: a tag slug that stops collapsing casing splits one tag into two pages; a broken `{{#located}}` publishes "0 of them carry a location"; a word counter that counts iframes misreports reading time |
...
- **`max-age`** is only what a browser holds. It buys nothing at the origin, and it's how long the
  page can be stale.

They used to be one number, tying page freshness to traffic-spike cost. Live endpoints
(`/moving.json`, `/reading.json`, `/watching.json`, and their `/now.json`s) now sit
